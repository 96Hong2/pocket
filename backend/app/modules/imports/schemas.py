...
# 한 번에 받아 주는 줄글 길이.
#
# 은행 앱 거래내역 한 줄이 계좌명·잔액까지 붙으면 80 자를 넘는다. 1000 자이던 시절에는
# 열두 줄쯤에서 **말없이** 잘렸고, 뒤쪽 거래가 왜 안 읽혔는지 알 방법이 없었다.
# 후보 20건 상한(service.MAX_CANDIDATES)은 넘칠 때 화면이 몇 건이 남았는지 적어 주므로,
# 길이를 넉넉히 두고 그쪽 안내에 맡기는 편이 낫다. 화면 상한도 같은 값이다
# (frontend/src/shared/lib/limits.ts 의 NL_TEXT_MAX_LENGTH).

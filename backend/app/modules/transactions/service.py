...
#
# 상호 길이(120)와 같은 값으로 둔다. 60 이던 시절에는 문자로 온 결제 알림 한 줄을
# 그대로 붙여넣으면 60 자를 넘겨 422 가 났고, 화면은 「기록을 불러오지 못했어요 · 다시 시도」
# 만 내놓아 몇 번을 눌러도 안 풀렸다. 검색어를 지우는 것이 답인데 그 말이 어디에도 없었다.
# 화면도 같은 값으로 잘라 두어(frontend/src/shared/lib/limits.ts) 이제 서버까지 오지 않는다.

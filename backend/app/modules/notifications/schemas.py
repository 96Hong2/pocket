...
**빈도(`frequency`)는 발송기가 읽지 않는다.** 알림은 하루 한 번으로 못 박혀 있고(ADR-0013),
컬럼이 초기 스키마에 남아 응답에만 싣는다. 값 목록은 모델의 enum 을 그대로 쓴다(여기 다시
적지 않는다).

시각은 `"20:00"` 모양의 문자열이다. 초를 싣지 않는다. 화면의 시각 입력이 분까지만
...
class NotificationSettingsOut(BaseModel):
    ...
    # 발송기가 읽지 않는 값. 하루 한 번 고정이라 응답에만 싣는다.

...
- 배너는 자리마다 하나. 전면 광고는 「예측할 수 있고 잠깐 멈춰도 되는 자리」에만 (ADR-0023).
  자리는 여섯이고 전부 관리 탭에서 눌러 들어가는 하위 화면이다 (ADR-0029). **예고를 적어 둘
  자리가 없으면 광고도 안 세운다** (ADR-0028).
- 전면 광고 총량은 `features/ads/useInterstitial.ts` 한 곳에서 센다: 한 세션 1회 · 하루 2회.
  화면이 `useFullScreenAd` 를 직접 부르지 않는다.
- 사람이 스스로 광고와 맞바꾸겠다고 누르는 자리는 리워드 광고(`useRewardedAd`)로 하고 상한을
  안 센다 (ADR-0024). 지금은 생활비 계산기 입구 셋과 **사진 한 장 받는 자리**(`usePhotoRewardedAd`·`VITE_AD_PHOTO_GROUP_ID`)다. 그룹이 전면과 다르다.
- 리워드 결과는 `earned`(끝까지 봄)와 `watched`(중간에 닫음)를 합치지 않는다.

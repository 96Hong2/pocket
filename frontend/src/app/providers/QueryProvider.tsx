 ...
 *
 * **한 번만 다시 부른다.** 예전에는 두 번이었는데, 서버에 아예 닿지 않는 자리에서
 * 10초짜리 마감이 세 번 돌아 30초를 넘겼다. 검수가 「최초 접속 시간 20초 초과」 로
 * 막은 자리다. 한 번이면 최악이 6 + 1 + 6 = 13초이고, 그 뒤에 「다시 시도」 가 뜬다.
 * 순간 끊김은 한 번으로도 대부분 넘어간다.
 */
 ...
 * - retry 1회 + 지수 백오프: 순간 끊김은 조용히 넘기고, 오래 끌지 않는다.

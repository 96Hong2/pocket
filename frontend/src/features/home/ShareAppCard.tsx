...
import { appLine, ShareInviteCard } from '../share';
 ...
 *
 * **닫을 수 있다.** 닫으면 다시 뜨지 않는다. 앱을 알릴 생각이 없는 사람에게 이 카드가
...
export function ShareAppCard({ onDismiss }: { onDismiss: () => void }) {
  ...
  return (
    <ShareInviteCard
      ...
      where="home"
      icon="29_chat"
      title="가계부 쓰기 싫어하는 친구, 있죠?"
      lead="10초면 한 건 끝난다고 알려 주세요"
      label="10초 가계부 친구에게 공유하기"
      message={appLine()}
      dismissLabel="공유 안내 닫기"
      onDismiss={() => {

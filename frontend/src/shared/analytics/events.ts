...
export const EVENTS = {
   ...
   *
   * **이 로그가 이번 확인 창의 유일한 성적표다.** 물었을 때 「계속 쓰기」를 고른 비율이
  ...
  /**
   * 지난번에 광고가 뜬 채로 끝났다. **앱을 열 때 한 번** 센다.
   *
   * 토스가 띄우는 광고는 네이티브라 우리에게 닫는 함수가 없다. 거기 갇힌 사람은 답을
   * 기다리지 않고 앱을 끄고, 그러면 결과가 아무 데도 안 남는다. 그래서 뜨는 순간 표를
   * 적고 끝날 때 지운다. 다음에 열었을 때 표가 남아 있으면 그 판이 갇힌 판이다.
   *
   * ⚠ 광고를 보다가 그냥 앱을 끈 사람도 섞인다. 고장만 세는 것이 아니다.
   * 자리(`where`)별로 갈라 보고 **튀는 자리**를 찾는 데 쓴다.
   */

...
export const EVENTS = {
  ...
  /**
   * 사진 장수가 오갔다. 받았나(`earned`) 썼나(`spent`), 못 받았으면 왜인가.
   *
   * **이 셋을 같이 봐야 장치가 제 구실을 하는지 알 수 있다.** 받은 것보다 쓴 것이 훨씬
   * 많으면 하루에 채워 주는 3장 안에서 다들 끝난다는 뜻이고, 그러면 수익이 아니라
   * 값에 천장을 씌우는 장치로만 도는 것이다. 반대로 `blocked` 가 잦으면 3장이 모자라
   * 사진으로 적기를 포기한 사람이 그만큼 있다는 뜻이라 선을 다시 볼 때다.
   *
   * 남은 장수(`left`)를 함께 싣는다. 몇 장쯤에서 사람들이 광고를 보기로 하는지가
   * 이 값 없이는 안 보인다.
   */

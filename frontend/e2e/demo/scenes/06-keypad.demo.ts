...
/** 기록 방법 네 가지 중 아직 안 열린 둘. 줄글은 M4 에서 열렸다. */
const LOCKED_TABS = ['캡처', '영수증'] as const;
...
test('09 키패드로 금액을 찍는 규칙', async ({ home, recordSheet, demo }) => {
  ...
  await demo.step('기록 방법은 네 가지. 지금 열려 있는 것은 키패드와 줄글이다');
  ...
  for (const label of LOCKED_TABS) {
    // 캡처·영수증은 다음 마일스톤 자리다. 눌리지 않게 막아 두고 자리만 보여준다.

 ...
 *
 * 순서는 카드가 하는 말 그대로다. 먼저 카드가 뜨는 조건(사흘째 빔)을 만들고,
 ...
 * 6건이 온다. 그래서 이 장면이 보여주는 것은 '얼마나 잘 알아듣는가' 가 아니라
...
/**
 * 캡처 스텁이 늘 내는 6건 중 서버가 스스로 켜 주는 넷(`backend/app/integrations/llm/stub.py`).
 * 확신이 낮은 카카오T 는 꺼진 채로, 카드 캐시백은 환불이라 못 켜는 채로 온다.

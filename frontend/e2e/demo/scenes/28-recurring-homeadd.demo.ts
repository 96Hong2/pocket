 ...
 * 권유다. 영상에서도 그 둘이 어떻게 다른지가 보여야 한다. 권유 카드는 한 번에 하나라
 * 저녁 알림은 홈 화면 추가가 물러난 뒤에 선다.
 */
...
test('57 매달 나가는 돈을 미리 적어 두고, 홈에 앱을 놓는다', async ({
  ...
  await demo.step('꼭 적을 것은 무엇이·얼마·며칟날 셋뿐이다');
  ...
  await demo.step('권유 카드는 한 번에 하나다. 저녁 알림은 이 카드가 물러날 때까지 기다린다');
  // 순서는 HomePage.tsx 의 규칙 그대로다. 홈 화면 추가 → 저녁 알림.

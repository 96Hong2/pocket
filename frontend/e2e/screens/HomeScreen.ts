...
class TodaySection {
  ...
  get emptyButton(): Locator {
    /*
      ...
      기록이 있는 날 아래에 서는 버튼은 「N 기록 더하기」라 여기에 걸리지 않는다.
      둘이 같은 이름이었다면 「이제 빈 날이 아니다」 를 보는 단언이 헛돌았다.
    */
    return this.root.getByRole('button', { name: /기록하기$/ });
  }

  /**
   * 이미 적은 지난 날의 목록 맨 아래에 서는 기록 입구. 「어제 기록 더하기」.
   *
   * 카드 밖, 「전체 내역 보기」 아래다. 오늘에는 없다.
   */
  get addMoreButton(): Locator {
    return this.root.getByRole('button', { name: /기록 더하기$/ });
  }

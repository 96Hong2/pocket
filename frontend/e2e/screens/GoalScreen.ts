...
class GoalFormSheet {
   ...
   * 기한 입력칸.
   *
   * 이제는 라벨이 `htmlFor` 로 칸 하나만 가리킨다(공용 `DateField`). 그래도 이름을
   * 못 박지 않는다. 「(선택)」 꼬리표가 붙어 있어 정확히 맞추려면 문구를 여기 또 적게 된다.
   */
   ...
   * 기한을 비우는 ✕.
   *
   * **값이 있을 때만 선다.** 달력을 한 번 열면 「재설정」 을 눌러도 칸에 값이 남는 기기가
   * 있어(iOS), 이 버튼이 없으면 「선택」 이라고 적힌 칸을 비울 방법이 사라진다.
   */
  get deadlineClearButton(): Locator {
    return this.root.getByRole('button', { name: '기한 지우기', exact: true });
  }

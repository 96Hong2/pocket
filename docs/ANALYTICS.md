...
| 전면 광고가 어느 자리에서 걸리나 | `interstitial_result` | 자리(**`closing` 하나뿐이다**)와 결과(`watched`·`skipped`), 지나간 이유(`no_group`·`unsupported`·`failed`·**`capped`**) |
...
| 목표를 세우기만 하나 실제로 모으나 | `goal_changed` · `goal_contributed` | 만듦·고침·지움과 기한을 걸었나(`deadline`)·종잣돈이 있었나(`seeded`), 모을 때 지난 날로 넣었나(`backdated`). **이름과 금액은 싣지 않는다** |
| 못 찾아서 찾아보나 | `search_used` | 검색어 길이와 찾은 건수(`hits`). **검색어는 싣지 않는다** |
...
**`interstitial_result` 의 자리가 하나로 줄었다(2026-09-21, ADR-0028).** 자산 탭에 들어올 때와
리포트에서 옛날 달을 세 번 훑었을 때가 더 있었는데, 둘 다 **사람이 광고를 부른 적이 없는
자리**였다. 탭을 눌렀더니 광고가 뜨고 달을 넘기다 광고가 뜬다. 토스 노출 가이드가 「광고가
나오는 시점과 광고를 본 뒤 얻는 내용을 미리 알리라」 고 하는 것이 정확히 그 모양이라 뺐다.
결산만 남은 이유는 **누르는 버튼이 있어서**다. 버튼에 적어 둘 자리가 있으면 예고가 되고,
없으면 기습이 된다. 그래서 `where` 값이 `closing` 하나로 줄었고, 옛 판의 `assets`·
`report_months` 는 더 안 들어온다. 그 자리에서 결산을 실제로 연 비율이 광고 없이 지나간
사람보다 눈에 띄게 낮으면 이 한 자리도 다시 볼 때다. `capped` 는 상한(한 세션 1회 · 하루 2회)에
걸려 아예 안 부른 것이다.
...
뜻이다. 그중에서도 `hits: 0` 이 값어치 있다. 검색어는 상호와 메모가 그대로 들어 있어 절대

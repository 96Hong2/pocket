...
**홈에서 스스로 서는 카드는 한 번에 둘까지다.** 누가 서고 누가 비키는지는 `HomePage` 한 곳에서
정한다. 순서는 곧 나갈 돈 → 홈 화면 추가 → 저녁 알림 → 예산 제안 → 공유다. 앞의 둘(한 쌍인
한 번뿐인 안내)이 서면 예산 제안이 비킨다. **예산 제안은 안 사라지고 기다리지만 한 번뿐인

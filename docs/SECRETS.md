...
**스텁은 캡처 이미지를 한 바이트도 읽지 않는다.** 어떤 사진을 올려도 정해 둔 5건이 나온다.
그래서 지금 나가는 캡처 요청은 어디에도 도달하지 않고, 위 §4 의 유출 위험은 **실제 vision
provider 를 붙이는 순간부터** 생긴다. 그 전에 계약을 확인한다.
...
### ⚠ 캡처 경로에서는 가리기가 성립하지 않는다

바로 위 규칙은 **줄글에서만 지켜진다.** `redact()` 는 문자열 전용이고, 캡처 경로에는 가리는
...
- 그래서 **provider 를 고를 때의 계약 검토 항목으로 올린다.** 최소한 zero-retention(요청 데이터를
  ...
  하지 않는다.
- `parse_usages.redacted_count` 가 캡처에서 늘 0 인 것이 이 사실의 기록이다. 다만 이 값으로 두
  경로를 가를 수는 없다. 가릴 것이 없던 평범한 줄글도 0 이다. 경로는 `source` 로 가른다.

| 이름 | 값 | 비고 |
|---|---|---|
| `NL_PARSE_DAILY_LIMIT` | 기본 300 | 하루에 받아 주는 분석 횟수. **줄글과 캡처가 이 상한을 함께 쓴다**(429 문구만 갈린다). 비밀값은 아니다. 넘으면 429 `USAGE_LIMIT` 이고 키패드 기록은 그대로 된다. 캡처 단가가 커서 provider 를 붙일 때 갈라야 할 값이다 |

...
> **콘솔에서 직접 발급한다.** 앱을 고른 뒤 왼쪽 메뉴 **[mTLS 인증서] › [+ 발급받기]** 다.
> (2026-09-09 콘솔에서 확인. 예전에는 "공개 문서에 없어 문의 항목" 이라고 적었는데, 콘솔에 메뉴가 있다.)
> 공식 문서 「서버 API 이용하기」가 정한 것: 인증서·키 파일은 유출되지 않게 보관, **만료 전에 재발급**,
> 무중단 교체가 필요하면 **인증서를 두 개 이상 등록**해 둘 수 있다. 발급 버튼은 아직 누르지 않았다
> (개인키 파일이 내려오는 일이라 사용자가 직접 한다). 발급 뒤 이 절을 실제 파일 이름·형식으로 채운다.
>
...
- 사진 바이트를 로그에 찍지 않는다(`LlmImage.__repr__` 가 길이만 남기도록 막아 뒀다).
  캡처와 영수증이 같은 `app/api/images.py` 를 지나므로 이 방어도 한 자리다.
- **모델이 돌려준 상호도 가린다.** 나가는 이미지를 못 가리는 만큼, 화면에 찍힌 카드번호가
  상호로 되돌아오는 경로가 마지막 그물이다. `imports/service._to_row` 가 `redact()` 를 지난다.
  data URL 을 푸는 `app/api/images.py` 도 예외 메시지·트레이스백에 입력 조각을 넣지 않는다.
  base64 한 토막만 새어도 사진 원문이 복원된다.
...
### ⚠ 사진 경로에서는 가리기가 성립하지 않는다 (캡처·영수증 둘 다)

바로 위 규칙은 **줄글에서만 지켜진다.** `redact()` 는 문자열 전용이고, 사진 경로에는 가리는
단계가 아예 없다. 그래서 **화면에 찍힌 카드번호·계좌번호·잔액·다른 사람 이름이 이미지 그대로
provider 로 나간다.** 감출 것이 아니라 알고 여는 구멍이다.

**이 구멍은 이제 입구가 둘이다.** `POST /imports/capture`(앨범 캡처)와
`POST /imports/receipt`(카메라 영수증). 영수증은 종이라 카드번호 뒷자리·승인번호·매장 주소가
찍혀 있다. 새 이미지 입구를 열 때마다 이 목록에 더한다.
...
| 이름 | 값 | 비고 |
|---|---|---|
| `NL_PARSE_DAILY_LIMIT` | 기본 300 | 하루에 받아 주는 분석 횟수. **줄글·캡처·영수증이 이 상한을 함께 쓴다**(429 문구만 갈린다). 비밀값은 아니다. 넘으면 429 `USAGE_LIMIT` 이고 키패드 기록은 그대로 된다. 사진 단가가 커서 provider 를 붙일 때 갈라야 할 값이다 |
...
| `VITE_AD_FULLSCREEN_GROUP_ID` | 콘솔에서 발급한 운영 **전면** adGroupId | 위와 같다. 비면 결산이 광고 없이 열린다(오류 없음) |

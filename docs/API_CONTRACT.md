...
**금액 검색은 부분일치가 아니라 정확히 그 금액이다.** `q` 에서 쉼표·공백·꼬리의 `원` 을 뗀
나머지가 숫자뿐이면 `amount = <그 값>` 조건을 글자 조건들과 OR 로 함께 건다. 자릿수가
겹친다고 12,000 을 찾다가 112,000 이 나오면 검색이 아니라 훼방이라서다.
화면의 안내 문구도 그렇게 적혀 있다(「이름·태그·금액으로 검색」).
...
| PATCH | `/recurring/{id}` | 고치기. 분류·태그·결제수단만 `null` 이 「지운다」 다 |
...
- `day_of_month` 는 1~31 이다. **그 달에 없는 날짜는 말일로 당겨 본다.** 31일에 걸어 둔 구독이
  ...
  1월 28일로 적히기 때문이다.
- `/due` 는 **전날과 당일 이틀만** 준다(`app/domain/recurring.DUE_LEAD_DAYS`). 이미 적었거나
  「이번 달은 됐어요」 를 누른 회차는 빠진다. 판정 기준은 날짜가 아니라 **그 회차의 지출일**이라,
  전날에 적은 사람에게 당일 또 뜨지 않는다.

...
| POST | `/categories` | 내 카테고리 만들기. 바디는 `{"name": "반려동물", "icon_key": "16_paw", "kind": "expense"}` |
| PATCH | `/categories/{category_id}` | 내 카테고리 고치기. 보낸 필드만 바뀐다 |
| DELETE | `/categories/{category_id}` | 내 카테고리 지우기. 204 |

기본 카테고리 13개는 마이그레이션이 심는다(`user_id` 가 NULL 이라 모든 사용자에게 보인다).
지출 아홉 · **수입 셋(월급·용돈·기타 수입)** · 이체 하나다. 수입이 하나뿐일 때는 리포트의
수입 쪽이 늘 한 조각이었다. 옛 '수입' 행은 지우지 않고 '기타 수입' 으로 이름만 옮겼으므로
그 `id` 는 `uuid5('수입')` 인 채로 남아 있다. 지웠으면 그 분류로 적어 둔 거래의 분류가
통째로 빠진다(FK 가 `SET NULL`).
...
**종류는 만들 때만 정한다.** `kind` 를 안 보내면 `expense` 다. `income` 도 만들 수 있고,
`transfer` 는 422 로 막는다. 이체는 집계에서 빠지는 화살표 한 줄이라 늘릴 것이 없다.
`PATCH` 에는 `kind` 가 없다. 종류를 나중에 바꾸면 그 분류로 적어 둔 지난 거래가 종류와
어긋나고, 이미 본 리포트의 숫자가 달라진다(ADR-0015).

`sort_order` 는 서버가 종류를 보고 넣는다. 지출은 `USER_CATEGORY_SORT_ORDER`(85)로 기본
지출(10~80) 뒤 '기타'(90) 앞, 수입은 `USER_INCOME_SORT_ORDER`(105)로 기본 수입(100~102) 뒤
'이체'(110) 앞이다. 컬럼 기본값 0 을 그대로 쓰면 내가 만든 것이 '식비'보다 앞에 선다.
...
**되살리는 것은 종류가 같을 때뿐이다.** 지웠던 지출 '보너스' 를 수입 '보너스' 로 만들면
되살리지 않고, 지운 행의 이름만 아무도 못 쓰는 값으로 옮긴 뒤 새 행을 만든다.

`PATCH` 에서 **필드를 빼는 것과 `null` 을 보내는 것이 같다.** 둘 다 "이 값은 그대로 둔다" 는
뜻이다(`/preferences` 와 같은 규칙). 이름과 아이콘은 비워 둘 수 있는 값이 아니라 지우는 길을
두지 않았다. 목표의 `target_date`·알림의 `remind_at` 처럼 `null` 이 '지운다' 인 곳과 다르다.
...
**금액 바로 앞의 `+` 는 수입이라는 표시다.** 사이에 공백이 있어도 같다(`알바비 +150000`).
적는 사람이 직접 준 신호라 말버릇(`월급`·`입금`·`용돈`)보다 세다. `+` 는 상호에서 떼고 읽는다.

분류는 종류에 따라 다른 사전을 본다. 지출은 여덟 갈래, **수입은 월급·용돈** 둘이다.
이체·환불은 분류를 찾지 않는다. 이체는 집계 밖이고 환불은 대상의 분류를 따른다.
사전보다 **기억한 상호 규칙이 먼저**다. 단 규칙은 분류의 종류가 후보의 종류와 맞을 때만 붙는다
(「기억한 분류」 참고).
...
| 메서드 | 경로 | 하는 일 |
| --- | --- | --- |
| GET | `/merchant-rules` | 기억한 상호·분류 목록. 손으로 건 것 먼저, 그 안에서 자주 맞은 순 |
| POST | `/merchant-rules` | 상호 하나에 분류를 손으로 건다 |
| DELETE | `/merchant-rules/{rule_id}` | 규칙 하나 지우기 |

각 규칙은 `source` 로 어디서 왔는지 밝힌다. `learned` 는 앱이 저장하면서 기억한 것,
`manual` 은 사용자가 목록에서 직접 적어 넣은 것이다. 화면이 이 값으로 목록을 가른다.

`POST` 는 `{merchant, category_id}` 를 받는다. 상호는 정규화(전각·대소문자·띄어쓰기 제거) 후
견주므로 `스타 벅스` 와 `스타벅스` 는 같은 규칙이다. **같은 상호가 이미 있으면 새로 만들지 않고
그 규칙의 분류를 바꾸고 `manual` 로 표시한다**(상호 하나에 규칙 하나가 DB 제약이기도 하다).
지웠던 상호를 다시 걸면 되살아난다. 어느 경우든 201 로 답한다. 화면은 "걸어 뒀다" 하나만 알면
되고, 만들었는지 덮었는지는 물어본 적이 없다. 정규화 결과가 비면 422 다.

손으로 건 규칙은 `applied_count` 를 올리지 않는다. 이 값은 **실제로 몇 번 맞았는지**라,
적기만 한 규칙이 자주 쓰인 규칙보다 위에 서면 목록 순서가 거짓이 된다.

규칙은 **줄글·캡처·영수증** 셋 다 저장할 때 자동으로 쌓인다. 검토 목록을 저장하는 코드가 한 곳이라
입구가 달라도 같은 규칙이 쌓인다. 분류 우선순위는 `내 규칙 → 모델이 고른 이름 → 사용자 확인` 이다.
**공용 상호 사전은 아직 없다.**

쌓이는 것은 **지출과 수입**이다. 수입도 월급·용돈·기타 수입으로 갈리니 고른 것을 기억할 값이 있다.
이체는 분류가 하나뿐이고 환불은 대상의 분류를 따라가서 쌓지 않는다. 붙일 때는 **분류의 종류가
후보의 종류와 맞아야** 한다. `알바비 12000` 으로 기억한 지출 분류가 `알바비 +150000` 처럼
수입이 된 후보에 붙으면 안 되기 때문이다. 한 상호에 규칙은 하나라서 같은 상호를 다른 종류로
저장하면 나중 것이 앞의 것을 덮고, 덮인 종류의 후보는 모델이 고른 이름으로 돌아간다.
...
소비와 수입을 각각 준다. 화면 위 전환은 조회를 다시 하지 않고 이미 받은 두 목록을 바꿔 끼운다.

| 필드 | 무엇 |
| --- | --- |
| `expense_breakdown` · `expense_breakdown_total` | 소비 조각과 그 합 |
| `income_breakdown` · `income_breakdown_total` | 수입 조각과 그 합 |
